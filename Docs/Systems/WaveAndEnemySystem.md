...
- **Data-Driven Wave**: 웨이브 콘텐츠와 실행 로직 분리
- **Result Object**: 준비 결과와 실패 원인을 명시적으로 반환
- **Factory**: 적 생성 및 필수 컴포넌트 초기화 캡슐화
- **Observer**: 적 생명주기와 스테이지 진행의 결합도 완화
- **State Aggregation**: 스폰 상태와 생존 수를 조합해 웨이브 완료 판정
- **enum 기반 스킬 분기**: EnemySkillType과 대상·수치 타입에 따라 EnemySkill이 실행 경로를 선택
...
- TODO: 확인 필요 — QuestCategory.KillEnemy는 정의되어 있지만 적 사망 시 QuestRecieveReport를 호출하는 코드는 현재 검색 범위에서 확인되지 않았다.
